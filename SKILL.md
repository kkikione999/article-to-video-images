...
分镜脚本不再直接写“图片生成提示词”，而是先写**结构化镜头设计**，再由脚本根据镜头结构自动生成 prompt。
...
2. 为每段口播确定**视觉功能**：引入、解释、对比、举例、推进、收束。
3. 选择最合适的 `镜头类型`，而不是默认写成 PPT 页。
4. 写清楚主体、动作、构图、层次和文字策略，让画面可以被真正“设计”出来。
5. 先确定统一主风格，再给每镜定义局部变体，保证整体一致、局部有变化。
6. 在 `避免项` 中写出本镜头最怕出现的低质量模式，例如海报感、英文 UI、字太多、过于空。
7. 保存到 `04-storyboard/storyboard-*.md`，后续 prompt 由脚本自动生成。
